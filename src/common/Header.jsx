            ...
            <div className='container'>
                <div className='flex justify-between items-center'>
                    <ul className='lg:hidden block'>
                        <li className="relative z-[15]">
                            <a
                                ...
                                className="gap-3 relative text-primary-white lg:text-custom2xl lg:leading-7 text-lg flex items-center space-x-1 cursor-pointer after:absolute ease-in-out duration-300"
                                href="#">
                                ...
                                {isDropdownOpen && (
                                    <span
                                        className="absolute w-[182px] left-0 top-3 px-2 pt-[14px] pb-2 mt-7 bg-light-black border border-gray-border rounded-xl z-30"
                                        ref={dropdownRef}>
                        ...
                        <ul className='flex flex-col lg:flex-row gap-10 items-center lg:items-center z-10'>
                            ...
                            <li className='flex gap-2 items-center'>
                                {NAV_DATA_LINK.map((link, index) => (
                                    <li className='bg-icon cursor-pointer bg-cover bg-no-repeat bg-center py-5 px-6' key={index} onClick={toggleMenu}>
                                        <a
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="transition duration-300 transform hover:scale-105 flex items-center justify-center"
                                            href={link.href}>
                                            <Icon className='size-8 transition duration-300 transform hover:scale-105' iconName={link.icon} />
                                        </a>
                                    </li>
                                ))}
                            </li>
                    ...
                    <div className='lg:hidden block'>
                        <div
                            className={`menuList ${isMenuOpen ? "max-lg:left-0" : "max-lg:left-[-100%]"} z-10 gap-6 max-lg:w-full max-lg:fixed max-lg:justify-center max-lg:top-0 nav-gradient max-lg:transition-all duration-300 flex items-center max-lg:min-h-screen`}
                            ref={menuRef}>
                            <ul className="flex flex-col z-10 gap-6 mt-24">
                                ...
                                <li className="flex items-center gap-2">
                                    {SOCIAL_ICONS.map((icon) => (
                                        <li
                                            key={icon.name}
                                            className="bg-menu-bg cursor-pointer bg-[length:100%_100%] h-[67px] w-[77px] bg-no-repeat bg-center py-5 px-4"
                                        >
                                            <a className="flex justify-center transition duration-300 transform hover:scale-105" href={icon.href}>
                                                <Icon className="size-8 transition duration-300 transform hover:scale-105" iconName={icon.name} />
                                            </a>
                                        </li>
                                    ))}
                                </li>
